...
public class ModelServiceTest
{
    ...
    public void SetUp()
    {
        ...
        _service = new ModelService(_mockStorage.Object, _mockMutex.Object);
    }
    ...
    /*
    [TestMethod]
    public async Task UpdateDetailsAsync_Throws_WhenLocked()
    {
        // Arrange
        var id = Guid.NewGuid();
        _mockMutex.Setup(m => m.IsLocked(It.IsAny<int>(), out It.Ref<string?>.IsAny))
                  .Returns(true);

        // Act & Assert
        await Assert.ThrowsAsync<ModelLockedException>(
            async () => await _service.UpdateDetailsAsync(id, "alias", null, null, null, CancellationToken.None));
    }
    
    [TestMethod]
    public async Task UpdateDetailsAsync_ReleasesLock_AfterExecution()
    {
        // Arrange
        var id = Guid.NewGuid();
        _mockStorage.Setup(s => s.UpdateDetailsAsync(
                It.IsAny<Guid>(),
                It.IsAny<string?>(),
                It.IsAny<List<string>?>(),
                It.IsAny<string?>(),
        ...
        // Act
        await _service.UpdateDetailsAsync(id, "alias", null, null, null, CancellationToken.None);

        // Assert
        _mockMutex.Verify(m => m.ReleaseLock(id.GetHashCode(), "system"), Times.Once);
    }

    [TestMethod]
    public async Task DeleteAsync_Throws_WhenLocked()
    {
        // Arrange
        var id = Guid.NewGuid();
        _mockMutex.Setup(m => m.IsLocked(It.IsAny<int>(), out It.Ref<string?>.IsAny))
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ModelLockedException>(
            async () => await _service.DeleteAsync(id, CancellationToken.None));
    }

    [TestMethod]
    public async Task DeleteAsync_ReleasesLock_AfterExecution()
    {
        // Arrange
        var id = Guid.NewGuid();
        _mockStorage.Setup(s => s.DeleteByIdAsync(id, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(true);
